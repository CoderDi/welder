...
$(document).ready(function(){
  $(".header-city__current").click(function(){
    $(this).toggleClass("active").next(".header-city__drop").toggleClass("active");
  });

  $(".works-tab").click(function(){
    $(this).parents(".works-tabs").find(".works-tab").removeClass("active");
    $(this).addClass("active");
    $(".works-block").removeClass("active");
  ...
  $('input[type="range"]').on("input change", function(e){
    ...
    $('.js-teams').slick('slickGoTo', slideno-1 );
  });
  $(".js-team-arrow").click(function(){
    if ($(this).hasClass("prev")) {
      $(".js-teams").slick('slickPrev');
    } else {
      $(".js-teams").slick('slickNext');
    }
  });

  $(".js-difference-slider").slick({
    slidesToShow: 1, 
    arrows: false,
    infinite: false,
    fade: true
  });
  $(".difference-block__slider_arrow").click(function(){
    if ($(this).hasClass("prev")) {
      $(this).parents(".difference-block__slider").find(".js-difference-slider").slick("slickPrev");
    } else {
      $(this).parents(".difference-block__slider").find(".js-difference-slider").slick("slickNext");
    }
  });
  $(".difference-block__slider--all").each(function(){
    $(this).text($(this).parents(".difference-block__slider").find(".difference-block__slide").length);
  });
  $('.js-difference-slider').on('beforeChange', function(event, slick, currentSlide, nextSlide){
    $(this).parents(".difference-block__slider").find(".difference-block__slider--current").text(nextSlide+1);
  });

  $(".price-btn-all").click(function(){
    $(this).hide().parents(".price").find("tr:nth-child(n+6)").addClass("active");
  });

  $(".wie-nav__link").click(function(){
    $(".wie-nav__link").removeClass("active");
    $(".wie-block").removeClass("active");
